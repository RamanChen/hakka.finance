...
import { shortenAddress, getContract, getEtherscanLink } from "../../utils";
import {
  useTokenBalance,
  useTokenBalances,
  useETHBalances,
...
const VaultPage = (props) => {
  ...
  const hakkaTotalSupply = new BigNumber(
    totalSupplyTokenAmount?.raw.toString()
  );
  ...
  if (inputAmount && rewardTokens && hakkaTotalSupply) {
    ...
    Object.keys(rewardTokens).map((tokenAddress) => {
      ...
      if (
        hakkaTotalSupply
          .div(bignumber1e18)
          .isGreaterThanOrEqualTo(amountBigNumber)
      ) {
        let balanceBigNumber =
          tokenAddress === ETHADDRESS
            ? new BigNumber(
                ethBalance[GUILDBANK[chainId as ChainId]]?.raw.toString()
              )
  ...
  const noTokenError = useMemo(() => !pickedRewardTokensAddress.length, [
    pickedRewardTokensAddress,
  ]);
  ...
  if (tokenAllowance) {
    allowance = new BigNumber(parseFloat(tokenAllowance.raw.toString()))
      .div(bignumber1e18)
      .toNumber();
  }
  ...
  useEffect(() => {
    if (
      parseFloat(inputAmount) > hakkaTotalSupply.div(bignumber1e18).toNumber()
    ) {
      setTotalSupplyError("The amount is more than HAKKA total supply");
    } else {
      setTotalSupplyError("");
    }
  }, [inputAmount]);
        ...
        <div sx={styles.body}>
          <div sx={styles.infomationContainer}>
            ...
            <div sx={styles.contract}>
              ...
              >
                {chainId ? shortenAddress(GUILDBANK[chainId]) : ""}
              </span>
            </div>
            <p>
              ...
              {Object.keys(rewardTokens).map((tokenAddress) => {
                return (
                  <RewardListItem
                    ...
                    isDefaultToken={Object.keys(
                      VAULT_TOKENS[chainId || 1]
                    ).includes(tokenAddress)} // chainId
        ...
        <div sx={styles.knowMoreWrapper}>
          ...
          <div sx={styles.knowMoreRow}>
            ...
            <div
              ...
              onClick={() => {
                window
                  .open("https://hakka-finance.gitbook.io/hakka-wiki", "_blank")
                  .focus();
              }}
            >
